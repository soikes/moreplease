...
languages.sql = {
  ...
  keyword:
    /\b(?:ABORT|ACTION|ADD|AFTER|ALTER|ANALYZE|AS|ASC|ATTACH|AUTOINCREMENT|BEFORE|BEGIN|BIGINT|BINARY|BLOB|BOOLEAN|BY|CASCADE|CASE|CAST|CHECK|COLLATE|COLUMN|COMMIT|CONFLICT|CONSTRAINT|CREATE|CROSS|CURRENT_DATE|CURRENT_TIME|CURRENT_TIMESTAMP|DATABASE|DATE|DATETIME|DECIMAL|DEFAULT|DEFERRABLE|DEFERRED|DELETE|DESC|DESCRIBE|DETACH|DISTINCT|DOUBLE PRECISION|DROP|EACH|ELSE|END|ESCAPE|EXCEPT|EXCLUSIVE|EXPLAIN|FAIL|FETCH|FIRST|FLOAT|FOR|FOREIGN|FROM|FULL|FUNCTION|GROUP|HAVING|IF|IGNORE|IMMEDIATE|INDEX|INDEXED|INITIALLY|INNER|INSERT|INSTEAD|INT|INTEGER|INTERSECT|INTO|ISNULL|JOIN|JSON|KEY|LAST|LEFT|LIMIT|MEDIUMINT|MINUS|NATURAL|NO|NOCASE|NOTNULL|NULL|NULLS|NUMERIC|OF|OFFSET|ON|ORDER|OUTER|OVER|PARTITION|PLAN|PRAGMA|PRIMARY|QUERY|RAISE|REAL|RECURSIVE|REFERENCES|REINDEX|RELEASE|RENAME|REPLACE|RESTRICT|RETURNS|RETURNING|RIGHT|ROLLBACK|ROW|ROWNUM|RTRIM|SAVEPOINT|SELECT|SET|SMALLINT|STORED|TABLE|TEMP|TEMPORARY|TEXT|THEN|TIME|TIMESTAMP|TINYINT|TO|TOP|TRANSACTION|TRIGGER|UNION|UNIQUE|UPDATE|USING|UUID|VACUUM|VALUES|VARBINARY|VARCHAR|VIEW|VIRTUAL|WHEN|WHERE|WITH|WITHOUT|XML)(?=\b)/i,
  boolean: /\b(?:FALSE|NULL|TRUE)\b/i,
  number: /\b0x[\da-f]+\b|\b\d+(?:\.\d*)?|\B\.\d+\b/i,
  operator:
    /[-+*\/=%&\|~]|!=?|<<|>>|<(?:=>?|<|>)?|>[>=]?|\b(?:AND|ALL|ANY|BETWEEN|IN|IS|LIKE|NOT|EXISTS|OR|GLOB|MATCH|REGEXP)\b/i,
  punctuation: /[;[\]()`,.]/,
};
